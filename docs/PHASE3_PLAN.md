...
│   └── api/
│       ├── auth.py                   # NEW (S6): Bearer-key dependency + rate limit on mutating routes
...
│   └── test_auth.py                  # NEW (401 without/with bad key, mutating vs read-only, rate limit)
│
└── docs/
          ...
          └─> P3-S3 FILTERS + INTERLOCKS (feat/p3-s3-interlocks)      [pure functions over Snapshot; test hardest]
                 └─> P3-S4 CONTROLLER + ENGINE + ACTUATION (feat/p3-s4-controller)
                        ...
                        └─> P3-S5 PERSISTENCE + POLLER + API (feat/p3-s5-control-api)
                               ...
                               └─> P3-S6 AUTH + FIREWALL (feat/p3-s6-security)
                                      │                               [API key on mutating routes, ufw rules, ssh hardening]
                                      └─> P3-S7 WEB (feat/p3-s7-control-ui)
                                             └─> P3-S8 DOCS + DEPLOY (feat/p3-s8-docs)
```

### P3-S6 — auth + firewall (security hardening before actuation goes live)

The device doses acid once Phase 3 lands; actuation must not be open to anyone on the LAN. Minimal, dependency-free hardening **before** the control UI ships (the UI then sends the key from day one):

- **API key auth** on all *mutating* routes — `POST /actuators/*`, `POST /control/*` (mode, E-STOP) — via a FastAPI dependency checking `Authorization: Bearer <HYDRO_API_KEY>`. Key set in `.env` (generated by `install.sh` if absent, printed once); read-only routes (`/sensors`, `/topology`, `/health`, `/doses`, `/alerts`) stay open on the LAN. **Constant-time compare; 401 without leaking which part failed. No key configured + real mode ⇒ refuse to start; mock mode ⇒ warn.**
- **Rate limit** mutating routes (simple in-process token bucket; no new deps) — bounds API-driven pump abuse on top of the interlock caps.
- **Firewall**: extend `deploy/hardening.sh` — default-deny inbound; allow 8000 and SSH from the LAN/management subnet only; document VLAN isolation + "never port-forward 8000; use WireGuard/Tailscale for remote" in `deploy/README.md`.
- **SSH**: key-only (`PasswordAuthentication no`, `PermitRootLogin no`) in `hardening.sh`.
- **Defense in depth note:** interlocks, caps, and E-STOP remain authoritative *on the device* — a stolen key still cannot dose past `max_ml_per_day` or bypass hard limits.
- Out of scope here: HTTPS/local CA, users/roles, audit beyond the dose ledger — later phase.
...
| Auth | Mutating routes 401 without/with wrong key, 200 with key; read-only routes open; rate limit trips and recovers; real mode without a key refuses to start | httpx TestClient; env-driven fixtures. |
...
| P3-S6 auth + firewall | Bearer-key dependency + rate limit on mutating routes, hardening.sh firewall/ssh, install.sh key generation | **S** |
| P3-S7 web | ControlPanel in ZoneSection, EStop, DoseHistory, AlertBanner — client sends the API key | **M** |
| P3-S8 docs + deploy | control.md, authoring-guide + architecture updates, fail-safe + security notes in deploy/README | **S** |
...
13. **(New)** Mutating routes require a Bearer API key + rate limit before the control UI ships (P3-S6); firewall + ssh hardening in `hardening.sh`. Full auth (users/roles, HTTPS) stays a later phase; on-device interlocks remain the last line of defense.

## 9 · Explicit non-goals (unchanged from architecture.md)

PID/proportional control, EC auto-dosing, pH-up chemistry, real level-sensor hardware, hot-reload, email/push alerting, MQTT/cloud/multi-tenant, AI/ML, full auth (users/roles/HTTPS — Phase 3 ships a single API key only).
